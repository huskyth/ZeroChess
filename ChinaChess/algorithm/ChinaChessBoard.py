...
class ChinaChessBoard:
    ...
    def __init__(self):
        ...
        for row, line in enumerate(self.pieces):
            for col, chess_name in enumerate(line):
                if chess_name:
                    # 将创建的棋子添加到属性map中
                    self.pieces[row][col] = Chess(chess_name, row, col)
                else:
                    self.pieces[row][col] = None

    def get_chess(self):
        """获取所有的棋盘上的棋子对象列表"""
        return [chess for line in self.pieces for chess in line if chess]
    ...
    def is_game_end(self, current_player):
        if self.judge_attack_general(current_player):
            print("将军....")
            # 检测对方是否可以挽救棋局，如果能挽救，就显示"将军"，否则显示"胜利"
            if self.judge_win(current_player):
                print("获胜...")
                return 1 if current_player == RED_STRING else -1
        return NOT_END
    ...
    def judge_m_attack(self, attack_row, attack_col, general_row, general_col):
        ...
        if attack_row == general_row or attack_col == general_col:
            ...
            if col_length + row_length == 5:
                # 判断是否蹩马腿
                if col_length == 1:
                    if general_row < attack_row and not self.pieces[attack_row - 1][attack_col]:
                        return True
                    elif general_row > attack_row and not self.pieces[attack_row + 1][attack_col]:
                        return True
                elif col_length == 4:
                    if general_col < attack_col and not self.pieces[attack_row][attack_col - 1]:
                        return True
                    elif general_col > attack_col and not self.pieces[attack_row][attack_col + 1]:
                        return True
    ...
    def judge_delete_position(self, all_position, clicked_chess):
        ...
        for row, col in all_position:
            ...
            position_chess_backup = self.pieces[row][col]
            # 2. 挪动位置
            # 移动位置
            self.pieces[row][col] = self.pieces[old_row][old_col]
            # 修改棋子的属性
            self.pieces[row][col].update_position(row, col)
            # 清楚之前位置为None
            ...
            # 4. 恢复到之前位置
            self.pieces[old_row][old_col] = self.pieces[row][col]
            self.pieces[old_row][old_col].update_position(old_row, old_col)
            self.pieces[row][col] = position_chess_backup
